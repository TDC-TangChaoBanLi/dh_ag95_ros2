...
class DhAg95Hardware : public hardware_interface::ActuatorInterface {
 ...
 private:
  ...
  double percent_to_command_unit(double percent) const;
  int command_unit_to_percent(double command) const;
  ...
  std::string command_unit_{"normalized"};
  double stroke_m_{0.095};
  bool invert_position_{false};
  double write_deadband_{0.01};
  ...
  double last_cmd_position_{9999.0};
};
